...
const agentRegistry=readJson(path.join(root,'registry','agents.json'));
const agent=agentRegistry.agents.find(x=>x.agentId===goal.agentId&&x.enabled);if(!agent)fail(`agent unavailable ${goal.agentId}`);
...
const inputHashes={
  goalSha256:sha(readText(goalPath)),agentSha256:sha(agentMd),goalsSha256:sha(agentGoals),planSha256:sha(agentPlan),memoryIndexSha256:sha(memoryIndex),
  modelRegistrySha256:sha(readText(modelRegistryPath)),toolRegistrySha256:sha(readText(toolRegistryPath)),turnContractSha256:sha(readText(turnContractPath)),
  skills:Object.fromEntries(skills.map(x=>[x.file,sha(x.text)]))
};
...
function uniqueRoleCandidates(roles){
  const seen=new Set();
  return roles.filter(role=>{if(!role||seen.has(role))return false;seen.add(role);return true;}).map(modelForRole).filter(x=>x.model);
}
function registeredTool(){
  const tool=toolRegistry.tools.find(x=>x.enabled&&x.tool==='RUN_FEATURE_EXPERIMENT');
  if(!tool)fail('RUN_FEATURE_EXPERIMENT is not enabled');
  if(tool.readOnly!==true||tool.brokerAction!==false)fail('research tool must be read-only and non-broker');
  return tool;
}

function actionSemanticSchema(){
...
function normalizeActionSemantic(decision){
  ...
  const turn={
    schema:TURN_SCHEMA,goalId:goal.goalId,status:'ACTION_REQUIRED',reasoningSummary:checked.reasoningSummary,
...
async function callModel(messages,schema,choice){
  const response=await fetch(`${base}/api/chat`,{
    ...
    body:JSON.stringify({model:choice.model,messages,format:schema,think:false,stream:false,keep_alive:'10m',options:{temperature:0,num_ctx:contextTokens,num_predict:outputTokens}})
  });
...
async function runSemanticDecision({stage,runDir,schema,validate,baseMessages,candidates}){
  ...
  for(let candidateIndex=0;candidateIndex<candidates.length;candidateIndex+=1){
    ...
    for(let attempt=1;attempt<=2;attempt+=1){
      globalAttempt+=1;
      const correction=globalAttempt===1?null:[
        'Continue the SAME goal without changing any hypothesis or evidence.',
        `Previous response was invalid: ${lastError}`,
        'Return only the exact small semantic JSON object requested by this stage. Do not add schema, goalId, status, action envelope or evidenceRefs; runtime owns those fields.'
      ].join('\n');
      ...
      try{
        call=await callModel(messages,schema,choice);
        parsed=parseFirstJsonObject(call.text);
        if(!parsed.value)error=`JSON_PARSE_FAILED:${parsed.error||'unknown'}`;
        else{try{decision=validate(parsed.value);}catch(e){error=String(e?.message||e);}}
      }catch(e){error=String(e?.message||e);}
      const diag={stage,globalAttempt,role:choice.role,attempt,model:choice.model,modelSource:choice.source,doneReason:call?.body?.done_reason??null,rawText:call?.text??null,parseStrict:parsed.strict??null,parseRecovered:parsed.recovered??null,parseTrailing:parsed.trailing??null,error};
      diagnostics.push(diag);writeJson(path.join(runDir,`${stage.toLowerCase()}-${choice.role.toLowerCase()}-attempt-${attempt}.json`),diag);
      if(decision){
        if(globalAttempt>1)console.log(`[autonomous-smoke] OUTPUT_RECOVERED stage=${stage} role=${choice.role} attempt=${attempt} globalAttempt=${globalAttempt}`);
        return{decision,attempts:globalAttempt,diagnostics,role:choice.role,model:choice.model,modelSource:choice.source,escalated:candidateIndex>0};
      }
      lastError=error||'OUTPUT_INVALID';
      console.warn(`[autonomous-smoke] OUTPUT_INVALID stage=${stage} role=${choice.role} attempt=${attempt}/2 error=${lastError}`);
    }
  }
...
function executeAction(action,runDir){
  ...
  if(child.error)fail(`tool spawn failed ${child.error.message}`);
  if(child.status!==0)fail(`tool exit=${child.status} stderr=${String(child.stderr||'').slice(0,800)}`);
  let toolEvidence;try{toolEvidence=JSON.parse(String(child.stdout||'').trim());}catch{fail(`tool returned invalid JSON: ${String(child.stdout||'').slice(0,800)}`);}
  if(toolEvidence.schema!=='ResearchToolEvidence@1.0.0'||toolEvidence.profitabilityClaim!==false)fail('tool evidence schema invalid');
  const evidenceId=`EVIDENCE-${sha(toolEvidence).slice(0,16)}`;
  const wrapped={evidenceId,actionId:action.actionId,createdAt:nowIso(),evidence:toolEvidence};
  writeJson(path.join(runDir,`${evidenceId}.json`),wrapped);
  return wrapped;
}
function compactEvidenceForModel(wrapped){
  const e=wrapped.evidence;
  return{
    evidenceId:wrapped.evidenceId,tool:e.tool,featureId:e.featureId,parameters:e.parameters,datasetHash:e.datasetHash,
    eventCount:e.eventCount,validationStart:e.validationStart,effectiveSymbolCount:e.effectiveSymbolCount,excludedSymbols:e.excludedSymbols,
    discovery:e.discovery,validation:e.validation,profitabilityClaim:false
  };
}
...
const firstRun=await runSemanticDecision({
  stage:'ACTION_DECISION',runDir,schema:actionSemanticSchema(),validate:validateActionSemantic,candidates:roleCandidates,
  baseMessages:[
    {role:'system',content:'You are an evidence-bound autonomous local research agent. Return only the small semantic JSON object requested.'},
    {role:'user',content:[basePrompt(),'# ACTION DECISION','Read the human goal and available capability. Return only: tool, featureId, period, reasoningSummary. Derive period from the human goal. Do not invent evidence.'].join('\n\n')}
  ]
});
const actionDecision=firstRun.decision;writeJson(path.join(runDir,'semantic-action.json'),actionDecision);
const firstTurn=normalizeActionSemantic(actionDecision);writeJson(path.join(runDir,'turn-1.json'),firstTurn);
...
const evidence=executeAction(action,runDir);
console.log(`[autonomous-smoke] TOOL_PASS evidence=${evidence.evidenceId} events=${evidence.evidence.eventCount}`);
const compactEvidence=compactEvidenceForModel(evidence);
const completionCandidates=uniqueRoleCandidates([firstRun.role,'LOCAL_REASONER']);
const completionRun=await runSemanticDecision({
  stage:'COMPLETION_DECISION',runDir,schema:completionSemanticSchema(evidence.evidenceId,evidence),
  validate:value=>validateCompletionSemantic(value,evidence.evidenceId,evidence),candidates:completionCandidates,
  baseMessages:[
    {role:'system',content:'You are the same evidence-bound autonomous local research agent. Read the supplied evidence and return only the small semantic JSON object requested.'},
    {role:'user',content:[
      basePrompt(),
      '# PRIOR CANONICAL ACTION TURN',JSON.stringify(firstTurn,null,2),
      '# TOOL EVIDENCE',JSON.stringify(compactEvidence,null,2),
      '# COMPLETION DECISION',
      'Return only: evidenceId, observedEventCount, observedDiscoverySampleCount, observedValidationSampleCount, reasoningSummary, conclusion, nextResearch.',
      'The observed counts must exactly match the supplied evidence. This verifies that you actually read the evidence. Do not claim profitability.'
    ].join('\n\n')}
  ]
});
const completionDecision=completionRun.decision;writeJson(path.join(runDir,'semantic-completion.json'),completionDecision);
const finalTurn=normalizeCompletionSemantic(completionDecision,evidence.evidenceId,evidence);writeJson(path.join(runDir,'turn-2.json'),finalTurn);

const completedAt=nowIso();
const result={
  schema:RESULT_SCHEMA,status:'PASS',runId,goalId:goal.goalId,agentId:agent.agentId,primaryModelRole:primaryRole,primaryModel:primary.model,startedAt,completedAt,inputHashes,
  modelExecution:{
    actionDecision:{role:firstRun.role,model:firstRun.model,modelSource:firstRun.modelSource,escalated:firstRun.escalated,attempts:firstRun.attempts},
    completionDecision:{role:completionRun.role,model:completionRun.model,modelSource:completionRun.modelSource,escalated:completionRun.escalated,attempts:completionRun.attempts}
  },
  semanticActionDecision:actionDecision,firstTurn,toolEvidence:evidence,semanticCompletionDecision:completionDecision,finalTurn,
  checks:{
    goalRead:true,workspaceContextRead:true,skillRead:true,correctCapabilitySelected:true,whitelistedToolExecuted:true,evidenceReturnedToSameAgent:true,
    evidenceIdRead:true,eventCountRead:true,discoverySampleCountRead:true,validationSampleCountRead:true,canonicalTurnNormalization:true,completeReturned:true,profitabilityClaim:false
  },
  profitabilityClaim:false
};
const resultPath=path.join(runDir,'result.json');writeJson(resultPath,result);
console.log(`[autonomous-smoke] COMPLETE run=${runId} status=PASS actionRole=${firstRun.role} actionAttempts=${firstRun.attempts} completionRole=${completionRun.role} completionAttempts=${completionRun.attempts}`);
console.log(`[autonomous-smoke] RESULT_PATH=${resultPath}`);
